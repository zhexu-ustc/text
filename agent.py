...
class Agent():
    ...
    def log_img_to_wandb(self, label_imgs, cond_imgs, ref_imgs, pose_img, pred_imgs, step=-1, prefix=''):
        if WANDB_ENABLE and not self.args.debug:
            if step == -1:
                step = self.global_step
            ...
            for i in range(len(pred_imgs)):
                ...
                if pose_img is not None:
                    img_dict[f"{prefix}_sub_pose_img"].append(wandb.Image(ldm_tensor2img(pose_img[i], preprocess=True)))
        ...
        with T.no_grad():
            ...
            for batch_idx, inputs in enumerate(eval_loader):
                ...
                if batch_idx == 0:
                    ...
                    self.log_img_to_wandb(
                        label_imgs, cond_imgs, ref_imgs, pose_img, pred_imgs, prefix="val"
                    )
            ...
            if (
                ...
                try:
                    ...
                    pred_imgs = outputs["logits_img_seq"][:, :, 0]
                self.log_img_to_wandb(
                    label_imgs, cond_imgs, ref_imgs, pose_img, pred_imgs, prefix="train"
                )
