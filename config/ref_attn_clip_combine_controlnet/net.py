...
def inner_collect_fn(args, inputs, outputs, log_dir, global_step, eval_save_filename='eval_visu'):
    ...
    if not visu_save:  # for every different combination, we build a new dir to save
        ...
        sub_pose_save_path = os.path.join(eval_log_dir, 'sub_pose')
        ensure_dirname(sub_pose_save_path)
    ...
    for _, sample in enumerate(ld):
        ...
        if visu_save:
            ...
            ensure_dirname(ref_control_save_path)
            sub_pose_save_path = os.path.join(eval_log_dir_filename, 'sub_pose')
            ensure_dirname(sub_pose_save_path)
        ...
        if 'label_pose' in sample: # pose
            image = tensor2pil(sample['label_pose'], resize_img=args.pos_resize_img, img_target_size=ref_img_size)[0]
            try:
        ...
        if 'reference_img_controlnet' in sample:
            ...
            try:
                ...
                print(f'some errors happened in saving label_imgs: {e}')
        if 'sub_img' in sample:
            image = tensor2pil(sample['sub_img'], resize_img=args.pos_resize_img, img_target_size=ref_img_size)[0]
            try:
                image.save(os.path.join(sub_pose_save_path, prefix + postfix + '.png'))
            except Exception as e:
                print(f'some errors happened in saving sub_imgs: {e}')
